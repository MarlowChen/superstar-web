...
export const ShareDropdown = ({
  ...
  // 統一的按鈕樣式 (乾淨、無笨重圓圈、流暢的 Hover)
  const buttonBaseClass = "group flex w-full items-center gap-3 rounded-[10px] px-2.5 py-2 text-left text-[13px] font-medium text-gray-700 dark:text-gray-200 transition-all duration-200 hover:bg-black/5 dark:hover:bg-white/[0.08] outline-none";
  const iconWrapperClass = "flex h-5 w-5 items-center justify-center shrink-0 text-gray-500 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white transition-colors duration-200";

  return (
    <div className="fixed inset-0 z-[9999]" style={{ pointerEvents: "none" }}>
      <motion.div
        ref={dropdownRef}
        // 更平滑、更有阻尼感的 Apple 風格動畫
        initial={{ opacity: 0, scale: 0.96, y: -4 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.96, y: -4 }}
        transition={{ duration: 0.2, ease: [0.16, 1, 0.3, 1] }}
        // 頂級 Glassmorphism 樣式
        className="absolute min-w-[220px] overflow-hidden rounded-2xl border border-black/5 bg-white/90 p-1.5 shadow-[0_12px_40px_rgba(0,0,0,0.12)] backdrop-blur-2xl dark:border-white/[0.08] dark:bg-[#1C1C1F]/85 dark:shadow-[0_16px_40px_rgba(0,0,0,0.5)] ring-1 ring-black/5 dark:ring-white/5"
        style={{
      ...
      >
        ...
        {typeof navigator !== "undefined" && "share" in navigator && (
          <>
            <button onClick={handleNativeShare} className={buttonBaseClass}>
        ...
        <div className="space-y-0.5 pb-0.5">
          {shareData.imageUrl && (
            <button onClick={handleDownloadImage} className={buttonBaseClass}>
              <span className={iconWrapperClass}>
          ...
          <button onClick={handleCopyLink} className={buttonBaseClass}>
            <span className={iconWrapperClass}>
