...
import PSFYoursBanner from "./AieroneYoursBanner";
...
export default function PSFYoursPage() {
  ...
  //       }/task-sse/stream?userId=${userId}&taskId=${taskId}&limit=5`;
  ...
  //                 if (task.publishedImages.length === 4) {
  //                   setIsGenerating(false);
  //                   cleanupSSE();
  //                 }
  //               }
  ...
  //     });
  //   },
  //   [cleanupSSE, startHeartbeatMonitor, resetHeartbeatMonitor, updateTaskStatus]
  // );
  ...
  //     const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/task/add`, {
  //       method: "POST",
  ...
  }, [isImageViewerOpen, selectedImageIndex, viewingImages.length, handleNext, handlePrevious]);
  ...
  //             return { text: t("status_in_queue"), color: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300", icon: "⏳" };
  //           case TaskStatus.PROMPT_DELIVERING:
  //             return { text: t("status_prompt_delivering"), color: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300", icon: "📤" };
  //           case TaskStatus.AI_PROCESSING:
  //             return { text: t("status_ai_processing"), color: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300", icon: "🤖" };
  //           case TaskStatus.GENERATING:
  //             return { text: t("status_generating"), color: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300", icon: "🎨" };
  //           default:
  //             return null;
  //         }
  //       })();
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      ...
      <div className="w-full max-w-7xl mx-auto px-6 py-12">
        {/* <div className="bg-slate-800/80 rounded-2xl p-8 border border-purple-500/20 mb-12 backdrop-blur-sm" aria-labelledby="generator-heading">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <div>
              ...
              >
                ...
                <option value="988f6fdca24db156bcb38528">{t("model_style_transfer")}</option>
                <option value="68ed9c1cad7d2563378060ed">{t("model_super_fusion")}</option>
                <option value="68ed9c82cc3e600fe7f15ff2">{t("model_custom_project")}</option>
              </select>
            </div>
            ...
            <div className="lg:col-span-2">
              ...
              />
              <p id="prompt-help" className="sr-only">Press Enter to start generating</p>
            </div>

            <div>
              <label className="block text-white font-medium mb-2 text-sm" htmlFor="style-range">
                {t("style_weight")}：{(styleWeight * 100).toFixed(0)}%
              </label>
              <input
                id="style-range"
                type="range"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-700/50 text-white rounded-lg px-4 py-2 border border-slate-600 focus:border-purple-500 outline-none text-sm transition-all"
              >
                <option value="sketch">{t("preset_sketch")}</option>
                <option value="standard">{t("preset_standard")}</option>
                <option value="detail">{t("preset_detail")}</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-700/50 text-white rounded-lg px-4 py-2 border border-slate-600 focus:border-purple-500 outline-none text-sm transition-all"
              >
                <option value="sq-1024">{t("size_square")}</option>
                <option value="poster-832x1216">{t("size_portrait")}</option>
                <option value="banner-1344x896">{t("size_landscape")}</option>
              </select>
            </div>
          </div>
          ...
          {generatedTasks.length > 0 && (
            <div className="mt-6 space-y-6">
              <h3 className="text-white font-medium mb-3">{t("generation_history")}</h3>
                    ...
                    {task.publishedImages.map((image, idx) => (
                      <div key={image.id} className="relative aspect-square rounded-lg overflow-hidden bg-slate-800/50 border border-purple-500/20 group">
                        {image.url ? (
                          <Image
                            src={image.url}
                            alt={task.prompt ? `${task.prompt} (${idx + 1})` : `Generated image ${idx + 1}`}
                            fill
        ...
        {/* 作品集 Tabs */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-white">{t("customizable_models")}</h1>
        </div>
        <ModelGalleryTabs />
      </div>

      {/* 滿版 Pitch Overlay */}
      <FullscreenPitchOverlay open={isPitchOpen} onClose={() => setIsPitchOpen(false)} closeLabel={t("close")}>
        <PitchSection />
      ...
      {isImageViewerOpen && viewingImages[selectedImageIndex] && (
        ...
        >
          <button
            type="button"
            onClick={handleCloseViewer}
            className="absolute top-4 right-4 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white z-10 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-400"
            aria-label={t("close")}
          >
          ...
          {selectedImageIndex > 0 && (
            <button
              ...
              className="absolute left-4 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white z-10 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-400"
          ...
          {selectedImageIndex < viewingImages.length - 1 && (
            <button
              ...
              className="absolute right-4 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white z-10 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-400"
              aria-label={t("next")}
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <div className="relative max-w-6xl max-h-[85vh] w-full h-full flex items-center justify-center px-20" onClick={(e) => e.stopPropagation()}>
            <Image src={viewingImages[selectedImageIndex].url} alt={viewingImages[selectedImageIndex].prompt || "Generated image"} fill className="object-contain" priority />
          </div>
