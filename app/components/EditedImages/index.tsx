...
const EditedImages: React.FC = () => {
  ...
  const fetchEditedImages = useCallback(async (pageNum: number = 1) => {
    console.log('🔍 fetchEditedImages 被調用，頁面:', pageNum);
  ...
  // 處理刪除
  const handleDelete = useCallback(async (image: EditedImage, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    try {
      const response = await fetch(
        `/api/user/edited-images/${image.id}`,
        {
          method: "DELETE",
          credentials: "include"
        }
      ...
      setEditedImages(prev => prev.filter(img => img.id !== image.id));
